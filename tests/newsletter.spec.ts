...
test.describe("Newsletter Signup", () => {
  ...
  test("Newsletter form is visible in the Contact section on landing page", async ({ page, baseURL }) => {
    await page.goto(baseURL!);
    // We don't strictly need consent for visibility tests
    const contactSection = page.locator('[data-testid="contact"]');
    const newsletterForm = contactSection.locator('form[action*="buttondown.com"]');
  ...
  test("Newsletter form is visible at the bottom of blog posts", async ({ page, baseURL }) => {
    ...
    await expect(newsletterForm).toBeVisible();
    await expect(page.getByText("Praktycznie o testowaniu, procesach QA")).toBeVisible();
  });

  test("Clicking subscribe tracks event in Mixpanel", async ({ page, baseURL }) => {
    await page.goto(baseURL!);
    // This one REALLY needs consent and tracking to work
    await acceptConsentIfVisible(page).catch(() => {
        console.warn("Failed to accept consent banner in time, continuing anyway...");
    });
    ...
    const hasEvent = events.some(e => e.eventName === "Newsletter Signup Clicked");
